...
fn main() {
    ...
    let center_image_path = "path/to/your/image.png";
    let center_image = image::open(center_image_path).unwrap();
    ...
    final_image.copy_from(&image, 0, 0).unwrap();
    final_image.copy_from(&center_image, x_offset as u32, y_offset as u32).unwrap();

    // Save the final image
    let output_path = "path/to/your/output/image.png";
    final_image.save(output_path).unwrap();
    println!("QR code with center image saved to: {}", output_path);
}
